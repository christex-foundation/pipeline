...
<div class="mx-auto flex max-w-[1500px] flex-col items-start px-4 lg:flex-row lg:px-8">
  <div class="w-full max-md:w-[100%] lg:sticky lg:top-0 lg:w-[40%] lg:pr-4">
    ...
    <Card class="mt-8 w-full rounded-[20px] bg-lime-300 px-6 text-teal-950 max-md:mt-6 border-0">
...
<!-- <div class="relative">
  {#if showUpdatePopup}
    ...
    <dialog
      open
      class="absolute w-[400px] max-w-full rounded-lg bg-white p-8 shadow-lg bottom-40 left-20 z-20"
    >
      <form
        action="?/addUpdate"
        method="POST"
      ...
      >
        ...
        <label class="block mb-2 text-sm font-medium text-gray-700">
          Title
          <input type="text" name="title" class="w-full p-2 mt-1 border rounded-lg" required />
        </label>
        <label class="block mb-4 text-sm font-medium text-gray-700">
          Body
          <textarea
...
<div class="relative">
  {#if showUpdatePopup}
    <Dialog open onOpenChange={closeUpdatePopup}>
      <DialogContent class="w-96 max-w-[25%] sm:max-w-[20%] rounded-lg bg-white p-6 shadow-lg sm:left-[50%] sm:top-[50%] sm:translate-x-[-50%] sm:translate-y-[-50%] fixed">  
        <DialogHeader>
          ...
          >
            {#if isAddingUpdate}
              <span class="flex items-center justify-center gap-2">
                <!-- Fallback loading text if Icon component isn't available -->
                Adding Update...
              </span>
            {:else}
              Add Update
